...
switch ($action) {
    case 'upload':
        ...
        if($imageCheck){
        $message = '<p class="notice error left-indent">An image by that name already exists.</p>';
        } elseif (empty($invId) || empty($imgName)) {
        $message = '<p class="notice error left-indent">You must select a vehicle and image file for the vehicle.</p>';
        } else {
        ...
        if ($result) {
        $message = '<p class="notice success left-indent">The upload succeeded.</p>';
        } else {
        $message = '<p class="notice error">Sorry, the upload failed.</p>';
        }
        }
    ...
    case 'delete':
        ...
        if ($remove) {
        $message = "<p class='notice success left-indent'>$filename was successfully deleted.</p>";
        } else {
        $message = "<p class='notice error left-indent'>$filename was NOT deleted.</p>";
        }
    ...
    default:
        ...
        $imageDisplay = '<p class="left-indent error">Sorry, no images could be found.</p>';
        }
